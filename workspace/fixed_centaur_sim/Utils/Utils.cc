...
#define FOOT_SWING_CLEARANCE1 0.0
#define FOOT_SWING_CLEARANCE2 0.4
...
void FootSwingTrajectory<T>::computeSwingTrajectoryBezier(T phase, T swingTime) {
  _p = Interpolate::cubicBezier<Eigen::Matrix<T, 3, 1>>(_p0, _pf, phase);
  _v = Interpolate::cubicBezierFirstDerivative<Eigen::Matrix<T, 3, 1>>(_p0, _pf, phase) / swingTime;
  _a = Interpolate::cubicBezierSecondDerivative<Eigen::Matrix<T, 3, 1>>(_p0, _pf, phase) / (swingTime * swingTime);
