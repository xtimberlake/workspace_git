...
namespace GolDIdnani{
  ...
  */
  ...
  CanonicalBaseGVect<T>::CanonicalBaseGVect(unsigned int i, unsigned int n)
    : GVect<T>(static_cast<T>(0), n), e(i)
  { (*this)[e] = static_cast<T>(1); }
...
#include <stdexcept>
  ...
  {
    T sum = static_cast<T>(0), ssum = static_cast<T>(0);
        ...
        if (equal > 0)
          {
            ...
            tmp_rank[i] /= static_cast<double>(equal + 1);
          }
      }
  ...
  */
  ...
  {
    T abs_a = static_cast<T>(fabs(a)), abs_b = static_cast<T>(fabs(b));
    if (abs_a > abs_b)
      return abs_a * sqrt(static_cast<T>(1.0) + (abs_b / abs_a) * (abs_b / abs_a));
    else
      return (abs_b == static_cast<T>(0.0) ? static_cast<T>(0.0) : abs_b * sqrt(static_cast<T>(1.0) + (abs_a / abs_b) * (abs_a / abs_b)));
  }
  ...
  void svd(const GMatr<T>& A, GMatr<T>& U, GVect<T>& W, GMatr<T>& V)
  {
    ...
    g = scale = anorm = static_cast<T>(0.0);
        ...
        if (i < n - 1)
          {
            ...
            for (j = l; j < n; j++)
              V[i][j] = V[j][i] = static_cast<T>(0.0);
          }
        V[i][i] = static_cast<T>(1.0);
        g = rv1[i];
        l = i;
      }
        ...
        for (unsigned int its = 0; its < max_its; its++)
          {
            ...
            for (l = k; l >= 0; l--) // FIXME: in NR it was l >= 1 but there subscripts start from one
              { // Test for splitting
                ...
                if (static_cast<T>(fabs(rv1[l]) + anorm) == anorm)
                  {
                    flag = false;
                    break;
                  }
                if (static_cast<T>(fabs(W[nm]) + anorm) == anorm)
                  break;
              }
            if (flag)
              {
                ...
                for (i = l; i <= k; i++)
                  {
                    f = s * rv1[i];
                    rv1[i] *= c;
                    if (static_cast<T>(fabs(f) + anorm) == anorm)
                      break;
                    g = W[i];
                    h = dist(f, g);
                    W[i] = h;
            ...
            g = dist(f, static_cast<T>(1.0));
            f = ((x - z) * (x + z) + h * ((y / (f + sign(f)*fabs(g))) - h)) / x;
            c = s = static_cast<T>(1.0); // Next QR transformation
            for (j = l; j <= nm; j++)
              {
                i = j + 1;
                g = rv1[i];
  ...
  int lu(const GMatr<T>& A, GMatr<T>& LU, GVect<unsigned int>& index)
  {
    ...
    for (i = 0; i < n; i++)
      {
        ...
        for (j = 0; j < n; j++)
          val = std::max(val, static_cast<T>(fabs(LU[i][j])));
    ...
    if (LU[n - 1][n - 1] == static_cast<T>(0.0))
      std::logic_error("Error in LU decomposition: matrix was singular");
...
#define solve_gold lu_solve_gold
  ...
  void random(GMatr<T>& m)
  {
    for (unsigned int i = 0; i < m.nrows(); i++)
      for (unsigned int j = 0; j < m.ncols(); j++)
        m[i][j] = static_cast<T>(rand() / double(RAND_MAX));
  }
  ...
  */
  ...
  {
    GVect<T> tmp(static_cast<T>(0), m.ncols());
    for (unsigned int j = 0; j < m.ncols(); j++)
      for (unsigned int i = 0; i < m.nrows(); i++)
        tmp[j] += m[i][j];
    return tmp;
  ...
  {
    GVect<T> tmp(static_cast<T>(0), m.nrows());
    for (unsigned int i = 0; i < m.nrows(); i++)
      for (unsigned int j = 0; j < m.ncols(); j++)
        tmp[i] += m[i][j];
    return tmp;
  ...
  {
    GVect<T> tmp(static_cast<T>(1), m.ncols());
    for (unsigned int j = 0; j < m.ncols(); j++)
      for (unsigned int i = 0; i < m.nrows(); i++)
        tmp[j] *= m[i][j];
    return tmp;
  ...
  {
    GVect<T> tmp(static_cast<T>(1), m.nrows());
    for (unsigned int i = 0; i < m.nrows(); i++)
      for (unsigned int j = 0; j < m.ncols(); j++)
        tmp[i] *= m[i][j];
    return tmp;
  ...
  {
    GVect<T> res(static_cast<T>(0), m.ncols());
    for (unsigned int j = 0; j < m.ncols(); j++)
      {
        for (unsigned int i = 0; i < m.nrows(); i++)
          res[j] += m[i][j];
  ...
  {
    GVect<T> res(static_cast<T>(0), m.rows());
    for (unsigned int i = 0; i < m.nrows(); i++)
      {
        for (unsigned int j = 0; j < m.ncols(); j++)
          res[i] += m[i][j];
  ...
  GVect<T> var(const GMatr<T>& m, bool sample_correction = false)
  {
    ...
    for (unsigned int j = 0; j < m.ncols(); j++)
      {
        sum = static_cast<T>(0.0); ssum = static_cast<T>(0.0);
        for (unsigned int i = 0; i < m.nrows(); i++)
          {
            sum += m[i][j];
            ssum += (m[i][j] * m[i][j]);
  ...
  {
    GVect<T> res(static_cast<T>(0), m.nrows());
    double sum, ssum;
