 ...
 * @LastEditors: haoyun 
 * @LastEditTime: 2022-09-20 15:32:16
...
class SpatialInertia {
 public:
   ...
   * inertia. i.e., Ia = Xa_c_start * Ibody * Xc_a
   */
   ...
   */
  SpatialInertia flipAlongAxis(spatial::CoordinateAxis axis) {
    Mat4<T> P = getPseudoInertia();
    Mat4<T> X = Mat4<T>::Identity();
    if (axis == spatial::CoordinateAxis::X)
      X(0, 0) = -1;
    else if (axis == spatial::CoordinateAxis::Y)
      X(1, 1) = -1;
    else if (axis == spatial::CoordinateAxis::Z)
      X(2, 2) = -1;
    P = X * P * X;
